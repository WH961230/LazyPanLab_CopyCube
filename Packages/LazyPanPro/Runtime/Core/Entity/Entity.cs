...
#if UNITY_EDITOR
            //挂载实体行为查看器 便于运行时实时查看绑定的行为
            Transform viewerRoot = Cond.Instance.Get<Transform>(this, Label.BODY);
            if (viewerRoot == null) {
                viewerRoot = Prefab.transform;
            }

            EntityBehaviourViewer viewer = viewerRoot.GetComponent<EntityBehaviourViewer>();
            if (viewer == null) {
                viewer = viewerRoot.gameObject.AddComponent<EntityBehaviourViewer>();
            }

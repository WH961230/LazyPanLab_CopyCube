...
namespace LazyPan {
    public class LazyPanSupport : EditorWindow {
        ...
        public void OnStart(LazyPanTool lazyPanTool) {
            image = AssetDatabase.LoadAssetAtPath<Texture2D>($"Packages/evoreek.lazypan/Editor/Image/赞赏码.png");
        }
        ...
        public void OnCustomGUI(float areaX) {
            ...
            GUILayout.Label("@" + LazyPanTool.GetText("支持小标题"), style);
            GUILayout.EndHorizontal();
            ...
            if (GUILayout.Button($"❤@  {LazyPanTool.GetText("作者B站主页")}  @❤", style)) {
            ...
            GUILayout.BeginHorizontal();
            style = LazyPanTool.GetGUISkin("AButtonGUISkin").GetStyle("button");
            if (GUILayout.Button($"❤@  {LazyPanTool.GetText("给作者充个电")}  @❤", style)) {
            ...
            GUILayout.BeginHorizontal();
            style = LazyPanTool.GetGUISkin("AButtonGUISkin").GetStyle("button");
            if (GUILayout.Button($"❤@  {LazyPanTool.GetText("特别鸣谢")}  @❤", style)) {
        ...
        public class SupportFans : EditorWindow {
            private void OnGUI() {
                ...
                GUILayout.Label(LazyPanTool.GetText("特别鸣谢"));
                GUILayout.FlexibleSpace();
                GUILayout.EndHorizontal();
                ...
                GUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                GUILayout.Label(LazyPanTool.GetText("特别鸣谢备注"));
                GUILayout.FlexibleSpace();
                GUILayout.EndHorizontal();
